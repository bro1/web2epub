...
import lj.utils.net.http.Downloader;
...
public class SlashDotSite extends SiteSkeleton {
	...
	public void doWork() throws Exception {
		List<LinkInformation> linksToStories = getLinksToStories();

		HttpUtil http = new HttpUtil();
		XMLUtils x = new XMLUtils();

		Publication epub = EpubUtils.createPublication(
				"Slashdot",
				"http://bro1.centras.info/epub/slashdot"
		...
		for (LinkInformation storyLink : linksToStories) {
			...
			Thread.sleep(100);
			String storyWithJSEnabled = http.getUrl(storyLink
					.getUrlToDownload());
			Document doc = x.buildDocument(storyWithJSEnabled);

			String fullTitle = this.getFullTitle(doc);
			String plainUrl = this.getPlainUrl(doc);
			Element description = this.findDescription(doc.getRootElement());

			List<Element> commentsList = this.getComments(plainUrl);

			writeEpub(epub, fullTitle, description, commentsList, counter,
					new URL(plainUrl));

			setProgress((int) (counter / (double) linksToStories.size() * 100));
	...
	public List<Element> getComments(String plainUrl) throws Exception {
		...
		d.writeToFile("aaa.html", fileContent);
	...
	private String getPlainUrl(Document d) throws JDOMException, IOException {
		...
		Element plainCommentsLinkElement = x1.getElement(d,
				"//x:div[@class='commentBox']/x:noscript/x:small/x:a");
		String plainUrl = plainCommentsLinkElement.getAttribute("href")
				.getValue();
	...
	public static void writeEpub(Publication epub, String title,
			Element description, List<Element> commentList, int counter, URL url)
			throws Exception {
		...
		OPSResource main = epub.createOPSResource("OPS/main" + counter
				+ ".html");
		...
		TOCEntry mainTOCEntry = toc
				.createTOCEntry(title, mainDoc.getRootXRef());
		...
		for (Element commentElement : commentList) {
			com.adobe.dp.epub.ops.Element commentEpub = EpubUtils
					.convertDomToEpub(epub, mainDoc, commentElement, url);
	...
	@SuppressWarnings("unchecked")
	private void findComments(Element comments, List<Element> commentList)
		...
		for (Element e : (List<Element>) comments
				.getContent(filterElementsOnly)) {
	...
	private void removeCommentControls(Element comments, Matcher matcher)
		...
		Element commentControlElement = xp.getElement(comments,
				"//x:div[@id = 'comment_sub_" + commentID + "']");
		if (commentControlElement != null) {
			commentControlElement.getParent().removeContent(
					commentControlElement);
		}
	}
